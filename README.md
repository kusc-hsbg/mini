...
아바타로 돌아다니고, **가까이 가면 음성·영상이 자동 연결**되고, 회의실을 잠그고,
...
- **스페이스**: 고유 URL(`/s/slug`) 보유, 여러 개의 방(맵) 포함 (생성 시 광장/오피스/파크 3맵 자동 생성)
- **대형 프리셋 맵 3종** (80×50급, 기존 대비 면적 9배+): 타운 스퀘어 · 픽셀 오피스 · 선셋 파크
...
### 음성·영상·회의 (WebRTC P2P, 서버 비용 0원)
- **근접 자동 연결** (4.5타일), 같은 프라이빗 영역끼리 연결, DND 시 차단
- 화면 공유(여러 명 동시), **공유 화면 클릭 → 확대 + 실시간 주석(공동 드로잉)**
- **손들기**, 이모지 반응, **녹화**(내 화면+마이크 → webm 저장)
- 말하는 사람 초록 링 표시
...
> - **위치/채팅/이벤트 동기화** → Supabase Realtime (매니지드 WebSocket)
> - **음성/영상/화면공유** → WebRTC **P2P** (구글 공개 STUN) — 서버 트래픽 0
>
> ⚠️ P2P 특성상 일부 엄격한 사내망/NAT 환경에서는 영상 연결이 안 될 수 있습니다
> (TURN 서버가 필요하지만 무료 범위를 벗어나 미포함).
...
├─ src/hooks        useRoomChannel(Realtime) · useWebRTC(P2P) · useControlChannel(체크인)
...
- **이동 동기화**: 80ms broadcast + 보간. **presence**로 접속자/상태 동기화.
- **WebRTC**: perfect negotiation 패턴의 mesh, 시그널링은 Realtime broadcast.
