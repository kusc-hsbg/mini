...
- **스페이스**: 고유 URL(`/s/slug`) 보유, 여러 개의 방(맵) 포함 (생성 시 프리셋 맵 자동 생성)
- **프리셋 맵 9종**: 타운 스퀘어 · 픽셀 오피스 · 선셋 파크 · 그랑프리 서킷 · 비치 리조트 · 스타홀 갤러리(NPC 전시관) · 야외 카페 · 바다 요트 서킷 · 하늘 비행기 서킷
- 32px 타일 기반, 오브젝트 레이어, 전경(나무 캐노피/램프 글로우) 레이어, 미니맵(M), 줌

### 아바타·이동
- 커스터마이즈: PNG 캐릭터 헤드 12종 · 피부톤 · 상의/하의 색 · 모자 7종 · 표정 6종
- **자체 캐릭터 기본 적용** — 저장값이 없거나 과거 픽셀 기본값이어도 PNG 캐릭터로 렌더
